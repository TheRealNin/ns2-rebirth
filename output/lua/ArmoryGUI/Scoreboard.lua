...
function ScoreboardUI_GetNumberOfMarinesByType(marineType)
  
    CheckForReload()
    local numberOfMarines = 0
    
    Log("My type: " .. marineType)
    
    
    for index, playerRecord in ipairs(sortedPlayerData) do
        Log("Player data: " .. playerRecord.Status)
        if marineType == playerRecord.Status then
