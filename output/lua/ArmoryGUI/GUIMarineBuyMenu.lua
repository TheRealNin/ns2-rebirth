...
-- I know how dumb this looks, but you can't get the weapon slot of an item without an instance...
GUIMarineBuyMenu.kItemSlotForWeapon = { 
    ["rifle"] = Rifle:GetHUDSlot(),
    ["pistol"] = Pistol:GetHUDSlot(),
    ["axe"] = Axe:GetHUDSlot(),
    ["welder"] = Welder:GetHUDSlot(),
    ["shotgun"] = Shotgun:GetHUDSlot(),
    ["mine"] = LayMines:GetHUDSlot(),
    ["pulsegrenade"] = PulseGrenadeThrower:GetHUDSlot(),
    ["clustergrenade"] = ClusterGrenadeThrower:GetHUDSlot(),
    ["gasgrenade"] = GasGrenadeThrower:GetHUDSlot(),
    ["exo"] = 2,
...
local function UpdateItemsGUIScale(self)
    ...
    GUIMarineBuyMenu.kSelectorSize = GUIScale( Vector(215, 100, 0) ) * kSmallIconScale
    ...
    GUIMarineBuyMenu.kMenuWidth = GUIScale(193)
    GUIMarineBuyMenu.kPadding = GUIScale(8.5)

    GUIMarineBuyMenu.kEquippedWidth = GUIScale(128)

    GUIMarineBuyMenu.kBackgroundWidth = GUIScale(1000)
    ...
    GUIMarineBuyMenu.kItemDescriptionPaneSize = GUIScale( Vector(480, 480, 0) )
end
...
function GUIMarineBuyMenu:_InitializeBackground()
    ...
    self.content:SetColor(Color(0.0, 0.0, 0.0, 0.0))
...
function GUIMarineBuyMenu:_InitializeItemButtons()
    ...
    local slotNames = self.hostStructure:GetItemSlotNames() or ""
    
    for i=1, #slotNames do
        local menu = GetGUIManager():CreateGraphicItem()
        self.menus[i] = menu
        menu:SetPosition(Vector( (GUIMarineBuyMenu.kMenuWidth  + GUIMarineBuyMenu.kPadding)* ( i -1), GUIMarineBuyMenu.kResourceDisplayHeight, 0))
        menu:SetTexture(GUIMarineBuyMenu.kContentBgTexture)
        menu:SetSize(Vector(GUIMarineBuyMenu.kMenuWidth, GUIMarineBuyMenu.kBackgroundHeight - GUIMarineBuyMenu.kResourceDisplayHeight, 0))
        menu:SetTexturePixelCoordinates(0, 0, GUIMarineBuyMenu.kMenuWidth, GUIMarineBuyMenu.kBackgroundHeight)
        
        menu.menuHeader = GetGUIManager():CreateGraphicItem()
        menu.menuHeader:SetSize(Vector(GUIMarineBuyMenu.kMenuWidth, GUIMarineBuyMenu.kResourceDisplayHeight, 0))
        menu.menuHeader:SetPosition(Vector(0, 0-GUIMarineBuyMenu.kResourceDisplayHeight, 0))
        menu.menuHeader:SetTexture(GUIMarineBuyMenu.kContentBgBackTexture)
        menu.menuHeader:SetTexturePixelCoordinates(0, 0, GUIMarineBuyMenu.kMenuWidth, GUIMarineBuyMenu.kResourceDisplayHeight)
        
        menu:AddChild(menu.menuHeader) 
        
        
        menu.menuHeaderTitle = GetGUIManager():CreateTextItem()
        menu.menuHeaderTitle:SetFontName(GUIMarineBuyMenu.kFont)
        menu.menuHeaderTitle:SetScale(GetScaledVector())
        GUIMakeFontScale(menu.menuHeaderTitle)
        menu.menuHeaderTitle:SetFontIsBold(true)
        menu.menuHeaderTitle:SetAnchor(GUIItem.Middle, GUIItem.Center)
        menu.menuHeaderTitle:SetTextAlignmentX(GUIItem.Align_Center)
        menu.menuHeaderTitle:SetTextAlignmentY(GUIItem.Align_Center)
        menu.menuHeaderTitle:SetColor(GUIMarineBuyMenu.kTextColor)
        menu.menuHeaderTitle:SetText(slotNames[i])
        menu.menuHeader:AddChild(menu.menuHeaderTitle)
    ...
    for k, itemTechId in ipairs(itemTechIdList) do
        ...
        local mapName = LookupTechData(itemTechId, kTechDataMapName)
        
        local itemHUDSlot = GUIMarineBuyMenu.kItemSlotForWeapon[mapName] or 1
        local menu = self.menus[itemHUDSlot]
        
        menu.numChildren = menu.numChildren + 1
        local menuPosition = menu.numChildren
        
        local graphicItem = GUIManager:CreateGraphicItem()
        graphicItem:SetSize(GUIMarineBuyMenu.kMenuIconSize)
        graphicItem:SetAnchor(GUIItem.Middle, GUIItem.Top)
        graphicItem:SetPosition(Vector(-GUIMarineBuyMenu.kMenuIconSize.x/ 2, GUIMarineBuyMenu.kIconTopOffset + (GUIMarineBuyMenu.kMenuIconSize.y) * menuPosition - GUIMarineBuyMenu.kMenuIconSize.y, 0))
        graphicItem:SetTexture(kInventoryIconsTexture)
        graphicItem:SetTexturePixelCoordinates(GetSmallIconPixelCoordinates(itemTechId))
        ...
        graphicItemActive:SetPosition(Vector(selectorPosX, -GUIMarineBuyMenu.kSelectorSize.y / 2 +GUIMarineBuyMenu.kIconTopOffset, 0))
        ...
        local itemEquipped = GUIManager:CreateTextItem()
        itemEquipped:SetFontName(GUIMarineBuyMenu.kFont)
        itemEquipped:SetFontIsBold(false)
        itemEquipped:SetAnchor(GUIItem.Middle, GUIItem.Center)
        itemEquipped:SetPosition(Vector(0, 0, 0))
        itemEquipped:SetTextAlignmentX(GUIItem.Align_Center)
        itemEquipped:SetTextAlignmentY(GUIItem.Align_Center)
        itemEquipped:SetScale(fontScaleVector)
        GUIMakeFontScale(itemEquipped)
        itemEquipped:SetColor(GUIMarineBuyMenu.kTextColor)
        itemEquipped:SetText(Locale.ResolveString("EQUIPPED"))
        itemEquipped:SetIsVisible(false)
        
        graphicItem:AddChild(itemEquipped)  
        
        graphicItem:AddChild(costIcon)  
        
        menu:AddChild(graphicItem)
    ...
    self.menuHeaderTitle:SetText(Locale.ResolveString("PRIMARY"))
...
function GUIMarineBuyMenu:_UpdateItemButtons(deltaTime)
    ...
    if self.itemButtons then
        ...
        for i, item in ipairs(self.itemButtons) do
            ...
            item.Equipped:SetIsVisible(MarineBuy_GetHas( item.TechId ))
...
function GUIMarineBuyMenu:_InitializeContent()
    self.descriptionPane = GetGUIManager():CreateGraphicItem()
    self.descriptionPane:SetPosition(Vector(0, 0, 0))
    self.descriptionPane:SetColor( Color(0, 0, 0, 0.5))
    --self.descriptionPane:SetTexture(GUIMarineBuyMenu.kContentBgBackTexture)
    --self.descriptionPane:SetTexturePixelCoordinates(0, 0, GUIMarineBuyMenu.kItemDescriptionSize.x, GUIMarineBuyMenu.kItemDescriptionSize.y)
    self.descriptionPane:SetSize(Vector(GUIMarineBuyMenu.kItemDescriptionPaneSize.x , GUIMarineBuyMenu.kItemDescriptionPaneSize.y, 0))
    self.descriptionPane:SetLayer(kGUILayerPlayerHUDForeground1)
    self.descriptionPane:SetIsVisible(false)
    self.background:AddChild(self.descriptionPane)
    ...
    self.descriptionPane:AddChild(self.itemName)
    ...
    self.descriptionPane:AddChild(self.portrait)
    ...
    self.itemDescription:SetTextClipped(true, (GUIMarineBuyMenu.kItemDescriptionSize.x )/self.itemDescription:GetScale().x, (GUIMarineBuyMenu.kItemDescriptionSize.y - GUIMarineBuyMenu.kPadding)/self.itemDescription:GetScale().y)
    GUIMakeFontScale(self.itemDescription)
    
    self.descriptionPane:AddChild(self.itemDescription)
...
function GUIMarineBuyMenu:_UpdateContent(deltaTime)
    ...
    if techId then
        ...
        if (mouseY + GUIMarineBuyMenu.kItemDescriptionPaneSize.y > Client.GetScreenHeight()) then
            mouseY = mouseY - GUIMarineBuyMenu.kItemDescriptionPaneSize.y
        end
        self.descriptionPane:SetPosition(Vector(mouseX , mouseY, 0))
        
    end
    
    local contentVisible = techId ~= nil and techId ~= kTechId.None
    self.descriptionPane:SetIsVisible(contentVisible)
...
function GUIMarineBuyMenu:_InitializeResourceDisplay()
    ...
    self.resourceDisplayBackground:SetSize(Vector(GUIMarineBuyMenu.kBackgroundWidth - GUIMarineBuyMenu.kButtonWidth - GUIMarineBuyMenu.kPadding, GUIMarineBuyMenu.kResourceDisplayHeight, 0))
    self.resourceDisplayBackground:SetPosition(Vector(0, GUIMarineBuyMenu.kPadding, 0))
    self.resourceDisplayBackground:SetAnchor(GUIItem.Left, GUIItem.Bottom)
