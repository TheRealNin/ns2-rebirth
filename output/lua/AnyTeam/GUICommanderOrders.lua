...
function GUICommanderOrders:Update(deltaTime)
    ...
    if player then
        ...
        if not GetAreEnemies(player, firstEntity) and HasMixin(firstEntity, "Orders") then     
            ...
            prevScreenPosition = GetClampedScreenPosition(firstEntity:GetOrigin(), -90)
        end
    ...
    if newOrders[1] then    
        prevScreenPosition = GetClampedScreenPosition(newOrders[1]:GetOrderSource(), -90)    
    end
    ...
    for i = 1, newOrdersNum do
        ...
        local screenPosition = GetClampedScreenPosition(orderPosition, -90)
