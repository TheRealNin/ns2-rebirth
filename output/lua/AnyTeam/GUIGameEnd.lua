...
function GUIGameEnd:SetGameEnded(playerWon, playerDraw, playerTeamType, winningTeamNumber )
    ...
    Log("Team number: %s", winningTeamNumber)
    if winningTeamNumber == kTeam1Index then
        messageString = kTeam1Name .. " Wins!"
        Log(messageString)
    elseif winningTeamNumber == kTeam2Index then
        messageString = kTeam2Name .. " Wins!"
        Log(messageString)
    end
...
local function OnGameEnd(message)
    ...
    if localPlayer then
        ...
        local playerTeamNumber = localPlayer:GetTeamNumber()
        ...
        local playerWin = ( message.win == playerTeamNumber )
        local playerDraw = ( message.win == kNeutralTeamType )
        local winningTeamNumber = tonumber(message.win)
        Log("Winning team number: %s", winningTeamNumber)

        ClientUI.GetScript("GUIGameEnd"):SetGameEnded( playerWin, playerDraw, playerTeamType, winningTeamNumber)
