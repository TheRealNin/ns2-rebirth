...
function GetIsBuildLegal(techId, position, angle, snapRadius, player, ignoreEntity, ignoreChecks)
    ...
    if legalBuild then
        ...
        legalBuild = legalBuild and GetInfestationRequirementsMet(techId, legalPosition, player:GetTeamNumber())
        if not legalBuild then
    ...
    if legalBuild then
        ...
        if LookupTechData(techId, kTechDataNotOnInfestation, false) and GetIsPointOnInfestation(legalPosition, player:GetTeamNumber()) then
            legalBuild = false
        end
