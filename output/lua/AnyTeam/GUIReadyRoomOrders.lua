...
local kHoverAnimDistance = GUIScale(4)

local kRotationDuration = 5

local kTeamColors = 
{
    [kTeam1Index] = Color(kPrecursorTeamType),
    [kTeam2Index] = Color(kPrecursorTeamType)
}
...
function GUIReadyRoomOrders:SendKeyEvent(key, down)
...
function GUIReadyRoomOrders:Initialize()
    ...
    self.welcomeText:SetText(niceWords[math.random(1, #niceWords)])
...
function GUIReadyRoomOrders:Update(deltaTime)
    ...
    for index, currentVision in ipairs(unitVisions) do   
        ...
        local screenPosition = GetClampedScreenPosition(worldPosition, GUIScale(96))
