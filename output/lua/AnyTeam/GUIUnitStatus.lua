...
local kStatusBgTexture = { [kMarineTeamType] = "ui/unitstatus_marine.dds", [kAlienTeamType] = "ui/unitstatus_alien.dds", [kPrecursorTeamType] = "ui/unitstatus_alien.dds", [kNeutralTeamType] = "ui/unitstatus_neutral.dds" }
local kStatusFontColor = { [kMarineTeamType] = Color(kMarineTeamColorFloat), [kAlienTeamType] = Color(kAlienTeamColorFloat),[kPrecursorTeamType] = Color(kPrecursorTeamColorFloat), [kNeutralTeamType] = Color(1,1,1,1) }
...
function GUIUnitStatus:UpdateUnitStatusBlip( blipIndex, localPlayerIsCommander, baseResearchRot, showHints, playerTeamNumber )
    ...
    elseif blipData.IsSteamFriend then
        ...
        textColor = kNameTagFontColors[relationColor]
    end
    ...
    if alpha > 0 and healthFraction ~= 0 then
        ...
        if blipData.IsPlayer and isEnemy and not blipData.EvolvePercentage then
            ...
            updateBlip.HealthBarBg:SetColor( kHealthBarBgColors[relationColor] )
            updateBlip.HealthBar:SetColor( kHealthBarColors[relationColor] )
    ...
    if alpha > 0 and armorFraction ~= 0 then
        ...
        if blipData.IsPlayer and isEnemy and not blipData.EvolvePercentage then
            ...
            updateBlip.ArmorBarBg:SetColor(kArmorBarBgColors[relationColor])
            updateBlip.ArmorBar:SetColor(kArmorBarColors[relationColor])
        end
    ...
    if abilityFraction > 0 then
        ...
        if alpha > 0 then      
            ...
            updateBlip.AbilityBarBg:SetColor(kAbilityBarBgColors[relationColor])
            ...
            if blipData.IsWorldWeapon and GUIPickups.kShouldShowExpirationBars then
                ...
                if ammoBarColor then
                    ...
                    updateBlip.AbilityBar:SetColor(kAbilityBarColors[relationColor])
                end
            end
