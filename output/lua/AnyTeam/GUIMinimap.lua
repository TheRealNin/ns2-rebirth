...
local kTeamColors = { }
kTeamColors[kMinimapBlipTeam.Friendly] = Color(189/255, 100/255, 143/255, 1)
kTeamColors[kMinimapBlipTeam.Enemy] = Color(247/255, 120/255, 49/255, 1)
kTeamColors[kMinimapBlipTeam.Neutral] = Color(172/255, 199/255, 203/255, 1)
kTeamColors[kMinimapBlipTeam.Alien] = Color(172/255, 199/255, 203/255, 1)--Color(1, 138/255, 0, 1)
kTeamColors[kMinimapBlipTeam.Marine] = Color(172/255, 199/255, 203/255, 1)--Color(0, 216/255, 1, 1)
-- steam friend colors
kTeamColors[kMinimapBlipTeam.FriendAlien] = Color(172/255, 199/255, 203/255, 1)--Color(1, 189/255, 111/255, 1)
kTeamColors[kMinimapBlipTeam.FriendMarine] = Color(172/255, 199/255, 203/255, 1)--Color(164/255, 241/255, 1, 1)

kTeamColors[kMinimapBlipTeam.InactiveAlien] = Color(172/255, 199/255, 203/255, 1)--Color(85/255, 46/255, 0, 1, 1)
kTeamColors[kMinimapBlipTeam.InactiveMarine] = Color(172/255, 199/255, 203/255, 1)--Color(0, 72/255, 85/255, 1)

------ NEW COLORS
kTeamColors[kMinimapBlipTeam.Enemy] = Color(1, 138/255, 0, 1)
kTeamColors[kMinimapBlipTeam.Friendly] = Color(0, 216/255, 1, 1)
-- steam friend colors
kTeamColors[kMinimapBlipTeam.FriendEnemy] = Color(1, 189/255, 111/255, 1)
kTeamColors[kMinimapBlipTeam.FriendFriendly] = Color(164/255, 241/255, 1, 1)

kTeamColors[kMinimapBlipTeam.InactiveEnemy] = Color(85/255, 46/255, 0, 1, 1)
kTeamColors[kMinimapBlipTeam.InactiveFriendly] = Color(0, 72/255, 85/255, 1)
...
kInfestationColor[kMinimapBlipTeam.FriendEnemy] = Color(1, 1, 0, .25)
kInfestationColor[kMinimapBlipTeam.FriendFriendly] = Color(1, 1, 0, .25)
...
kInfestationColor[kMinimapBlipTeam.InactiveEnemy] = Color(0.2/3, 0.7/3, 0.2/3, .25)
kInfestationColor[kMinimapBlipTeam.InactiveFriendly] = Color(0.2/3, 0.7/3, 0.2/3, .25)
local kInfestationDyingColor = { }
kInfestationDyingColor[kMinimapBlipTeam.Friendly] = Color(1, 0.2, 0, .25)
kInfestationDyingColor[kMinimapBlipTeam.FriendEnemy] = Color(1, 0.2, 0, .25)
kInfestationDyingColor[kMinimapBlipTeam.FriendFriendly] = Color(1, 0.2, 0, .25)
...
kInfestationDyingColor[kMinimapBlipTeam.InactiveEnemy] = Color(1/3, 0.2/3, 0, .25)
kInfestationDyingColor[kMinimapBlipTeam.InactiveFriendly] = Color(1/3, 0.2/3, 0, .25)
...
function GUIMinimap:DrawMinimapName(item, blipTeam, clientIndex, isParasited)
    ...
    local drawName = self.spectating or IsFriendly(blipTeam)
    
    if drawName and self.showPlayerNames and clientIndex > 0 then
   -- Log("Trying to draw name")
        ...
        if record and record.Name then
            ...
            elseif self.spectating then
                if MinimapMappableMixin.OnSameMinimapBlipTeam(blipTeam) then
...
--Log("Not drawing name: spectating: %s, friendly: %s, showNames: %s, clientIndex: %s, blipTeam: %s", self.spectating,  IsFriendly(blipTeam),self.showPlayerNames, clientIndex, blipTeam)
    end
...
function GUIMinimap:InitMinimapIcon(icon, blipType, blipTeamType)
    ...
    icon:SetLayer(layer)
    icon:SetTexturePixelCoordinates(unpack(texCoords))
    icon:SetSize(icon.blipSize)
...
function GUIMinimap:Update(deltaTime)
    ...
    if self.background:GetIsVisible() then
        ...
        -- Player team is always blue on the minimap, enemies are always orange
        self.playerTeam = player:GetTeamNumber()
        ...
        if now > self.nextActivityUpdateTime then
            ...
            --Log("Show player names? %s", self.showPlayerNames)
            self.spectating = player:GetTeamNumber() == kSpectatorIndex
            self.clientIndex = player:GetClientIndex()
