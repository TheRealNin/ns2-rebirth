...
local function GetAutomaticOrder(self)
    ...
    if self.timeOfLastFindSomethingTime == nil or Shared.GetTime() > self.timeOfLastFindSomethingTime + 1 then
        ...
        if primaryTarget and (HasMixin(primaryTarget, "Weldable") and primaryTarget:GetWeldPercentage() < 1) and not primaryTarget:isa("MAC") then
            ...
            local constructablePowernodes = GetEntitiesWithMixinForTeamWithinRange("Construct", kNeutralTeamNumber, self:GetOrigin(), MAC.kOrderScanRadius)
            constructables = ConcatTable(constructables, constructablePowernodes)
            ...
            if not target then
                ...
                local weldablePowernodes = GetEntitiesWithMixinForTeamWithinRange("Weldable", kNeutralTeamNumber, self:GetOrigin(), MAC.kOrderScanRadius)
                weldables = ConcatTable(weldables, weldablePowernodes)
...
function MAC:OnOverrideOrder(order)
    ...
    if not GetAreFriends(self, orderTarget) then
        order.orderParam = -1
    end
