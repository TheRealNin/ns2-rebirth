...
function CHUDGUI_EndStats:CreateTeamBackground(teamNumber)
	...
	local teamName = kTeam1Name
    local teamType = kTeam1Type
    ...
    if teamType == kMarineTeamType then
        teamName = teamName .. " (Frontiersmen)"
    elseif teamType == kAlienTeamType then
        teamName = teamName .. " (Kharaa)"
...
function CHUDGUI_EndStats:Update(deltaTime)
	...
	if Shared.GetTime() > lastStatsMsg + kMaxAppendTime and (#finalStatsTable > 0 or #cardsTable > 0 or #miscDataTable > 0) and gameInfo then
		...
		self:SetTeamName(self.team1UI, miscDataTable.team1Name or kTeam1Name)
		self:SetTeamName(self.team2UI, miscDataTable.team2Name or kTeam2Name)
		...
		if #techLogTable > 0 or #buildingSummaryTable > 0 then
			...
			local team1Name = miscDataTable.team1Name or kTeam1Name
			local team2Name = miscDataTable.team2Name or kTeam2Name
...
originalGameEnded = Class_ReplaceMethod( "GUIGameEnd", "SetGameEnded",
	function(self, playerWon, playerDraw, playerTeamType, winningTeamNumber)
		originalGameEnded(self, playerWon, playerDraw, playerTeamType, winningTeamNumber)
