...
local function ConstructCachedTargetList(origin, forTeam, damage, radius, fallOffFunc, includeNeutral)

    local hitEntities
    if includeNeutral then
        hitEntities = GetEntitiesWithMixinForTeamWithinRange("Live", forTeam, origin, radius)
        local powerNodes = GetEntitiesWithMixinForTeamWithinRange("Live", kNeutralTeamNumber, origin, radius)
        hitEntities = ConcatTable(hitEntities, powerNodes)
    else
        hitEntities = GetEntitiesWithMixinForTeamWithinRange("Live", forTeam, origin, radius)
    end
    local targetList = {}
