...
local function UpdateTargets(self)
    ...
    local trackEntities = GetEntitiesForTeamWithinRange("Player", GetEnemyTeamNumber(PlayerUI_GetTeamNumber()), PlayerUI_GetOrigin(), kTrackEntityDistance)
    ...
    for t = 1, #trackEntities do
        ...
        if inFront and trackEntity:GetIsAlive() and not (trackEntity.GetIsCloaked and trackEntity:GetIsCloaked()) then
            ...
            if trace.entity == trackEntity then
                ...
                local scalar = max:GetLength() / distance * 4
                local size = CoordsToSize(kTargetingReticuleCoords)
    ...
    if closestToCrosshair ~= nil and closestDistToCrosshair < 50 then
        ...
        self.targetcrosshair:SetColor(Color(1, 1, 1, 1))
