    ...
    for index, player in ientitylist(players) do
        ...
        if relevant then
            ...
            if not healthColor then
                healthColor = kHealthColors[1]
                --Log("Couldn't get health color for team %s", player:GetTeamType())
            end
            local armorColor = kArmorColors[player:GetTeamType()]
            if not armorColor then
                armorColor = kArmorColors[1]
                --Log("Couldn't get armor color for team %s", player:GetTeamType())
            end
...
function GUIInsight_PlayerHealthbars:CreatePlayerGUIItem()
    ...
    return { Background = playerBackground, Name = playerNameItem, HealthBar = playerHealthBar, RegenBar = playerRegenBar, ArmorBar = playerArmorBar, HealthChangeBar = playerHealthChangeBar, EnergyBG = playerEnergyBackground, EnergyBar = playerEnergyBar, StoredValues = {TotalFraction = -1} }
end